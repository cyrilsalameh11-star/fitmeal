  ...
  return (
    <div className="space-y-10 lg:space-y-14">
      ...
      <div className="relative">
        ...
        <div ref={scrollRef} className="flex border-b border-gray-200 justify-start w-full gap-8 overflow-x-auto scrollbar-hide">
          {EXERCISE_TABS.map(tab => (
            <button
              key={tab.id}
              data-tab={tab.id}
              onClick={() => handleTabClick(tab.id)}
              className={`pb-4 text-sm font-bold uppercase tracking-wider whitespace-nowrap transition-all duration-300 relative ${
                activeTab === tab.id
                  ? 'text-gray-900'
            ...
            >
              ...
              {activeTab === tab.id && (
                <motion.div
                  ...
                  className="absolute bottom-[-1px] left-0 right-0 h-[2px] bg-gray-900 z-10"
                />
              )}
            </button>
          ))}
